...
;; wires are crossed to wrong segments
;; Sub knows signal, we observe segments and conclude intended mapping.  Assume no double
;; connections.

;;; lots of extraneous code in this file as I was exploring ideas.  Definitely, overthought
;;; the idea of matching individual segments to wires.  Really only need to recognize full
;;; digit patterns.  Deduction by counts and then shared segments of known patterns.
...
(def seven-segs (set [:a :b :c :d :e :f :g]))
...
;; get singles first
;; 1478 -- single assignment
;; [069] -- 4+7 picks 9, not 7 for 6, other 0
;; [235] -- 3 overlaps 7, 5 overlaps 6, other 2


(defn assign-1478 [patterns]
  (reduce (fn [m pat]
            (case (count pat)
              2 (assoc m 1 pat pat 1)
              3 (assoc m 7 pat pat 7)
              4 (assoc m 4 pat pat 4)
              7 (assoc m 8 pat pat 8)
              5 (update m 532 conj pat)
              6 (update m 960 conj pat)))
          {}
          patterns))

(defn assign-960 [assignments]
  (let [p7 (assignments 7)
        p4 (assignments 4)
        s960 (set (assignments 960))
        p9 (first (filter #(and (every? % p4) (every? % p7)) s960))
        p6 (first (filter #(not (every? % p7)) (disj s960 p9)))
        p0 (first (disj s960 p9 p6))]
    (assoc assignments 9 p9 p9 9 0 p0 p0 0 6 p6 p6 6)))

(defn assign-532 [assignments]
  (let [s532 (set (assignments 532))
        p7 (assignments 7)
        p3 (first (filter #(every? % p7) s532))
        p6 (assignments 6)
        p5 (first (filter #(every? p6 %) (disj s532 p3)))
        p2 (first (disj s532 p5 p3))]
    (assoc assignments 3 p3 p3 3 5 p5 p5 5 2 p2 p2 2)))

  
           

(def sample-seg-input
  "acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf" )

(defn ->seg [s]
  (reduce (fn [r ch] (conj r (keyword (str ch)))) #{} s))

;; ten signals, ignore two empties in the middle, four codes
(defn parse-line [line]
  (let [segs (str/split line #"\W")  
        signals (mapv ->seg (take 10 segs))
        code (mapv ->seg (drop 12 segs))]
    [signals code]))



(defn decode-line [line]
  (let [[signals code] (parse-line line)
        assignments (-> signals
                        assign-1478
                        assign-960
                        assign-532)]
    (map assignments code)))



(defn answer-part1 [input]
  (let [results (mapcat decode-line (str/split-lines input))
        groups (group-by identity results)]
    (reduce + (map count (map groups [1 4 7 8])))))

;; really should have changed decode line to return the 4-digit number as a single long

(defn answer-part2 [input]
  (reduce + (map #(reduce (fn [r d] (+ d (* r 10))) 0 %)
                 (map decode-line (str/split-lines input)))))




;; seg-groups {6 [0 6 9], 2 [1], 5 [2 3 5], 4 [4], 3 [7], 7 [8]}
...
;; then use overlaps to figure out related

;;; notice discrimators among the multi count groups
;;; overlaps with known numbers makes it easier
...
;;; Much of the following isn't used but it might inform the above code


;; vector of sets of segs, indexed by num [ #{:a ..} #{:b ...} ...]
(def segs (reduce (fn [r i] (conj r (set (segments i)))) [] (range (count segments))))

;; map of seg to possible nums  {:x #{2 4 5}}
(def segnums (reduce-kv (fn [m i sgs] (reduce (fn [mmm sg] (update mmm sg conj i)) m sgs))
                        (zipmap [:a :b :c :d :e :f :g] (repeat #{}))
                        segs))
  
;; inv map of segs, set of segs to num
(def invsegs (into {} (map-indexed (fn [i s] [s i]) segs)))

;; map of cnt to possible num
(def seg-groups (group-by #(count (segments %)) (range (count segments))))

;; The first ten are signals trying to do the 10 digits.  By count, you can deduce the
;; actual mapping.  Use that mapping to decode the last four digits.

(def single-count-nums
  (reduce-kv (fn [m k v] (if (= (count v) 1) (assoc m k (first v)) m))
             {}
             seg-groups))

(defn decode-line1 [line]
  (let [[signals code] (parse-line line)
        single-signals (filter #(single-count-nums (count %)) signals)]
    (println)
    (doseq [sng single-signals]
      (println "single " sng (single-count-nums (count sng))))))

(defn decode-line2 [line]
  (let [[signals code] (parse-line line)
        single-signals (filter #(single-count-nums (count %)) signals)]
    (println)
    (println "line" line)
    (doseq [sng signals]
      (if-let [single (single-count-nums (count sng))]
        (println " single   " sng single)
        (println " multiple " sng (seg-groups (count sng)))))))

(defn pr-line [line]
  (let [[signals code] (parse-line line)]
    (doseq [sig signals]
      (println sig (seg-groups (count sig))))
    (println " --- ")
    (doseq [c code]
      (println c (seg-groups (count c))))
    (println)))


(defn pr-decode-line [line]
  (let [[signals code] (parse-line line)
        single-signals (filter #(single-count-nums (count %)) signals)
        assignments (-> signals
                        assign-1478
    ...
    (println "line" line)
    (dotimes [n 10]
      (println n (signals n) (assignments (signals n))))
    (println "decoding")
    (dotimes [n 4]
      (println n (code n) (assignments (code n))))))
