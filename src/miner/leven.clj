...
;; SEM: pretty good, but not competitive with fastest
...
(defn levenshtein-distance
  ...
  (peek
   (reduce
    ...
    (mapv #(identity %2) (cons nil b) (range)) 
    a)))



;; SEM: fixed a bug on the `a` as empty string.  Need to use mapv instead of map to init the
;; reduce.
